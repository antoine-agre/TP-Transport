...
public class ParseurMetro {
	...
	public static void initialisisation_listeStation() {
		    try{
		    	//lecture en format utf-8
			  	...
			  	//recuperation des noms de station et creation des stations correspondantes
			  	for(String s : station) {//System.out.println(s);
	...
	static ArrayList<ArrayList<LocalTime>> plageHoraire(ArrayList<String> liste) {
		...
		try {
		  	...
		  	while(scanner.hasNextLine()){
				a = scanner.nextLine();
				System.out.println("ligne : " + a);
				System.out.println("ligne : %à partir de");
				System.out.println("test : " + a.equalsIgnoreCase("%à partir de"));
	...
	static void parseurMetro() {
		...
		for(String str: liste.get(id_depart)) {
			...
			if(j < liste.get(id_arriv).size()) {
				...
				Trajet e = new Trajet(listeStation.get(x), listeStation.get(y), MoyenTransport.METRO,
						Integer.valueOf(liste.get(id_duree).get(j)));
	...
	public static void main(String args[]){
		System.out.println(parseMetro("metro.txt"));
